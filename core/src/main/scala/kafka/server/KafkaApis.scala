  ...
  def handle(request: RequestChannel.Request) {
    try {
      ...
      ApiKeys.forId(request.requestId) match {
        ...
        /**
         * Consumer初次消费的时候，Consumer发送该请求，寻找该Group下的Coordinator
         */
        case ApiKeys.GROUP_COORDINATOR => handleGroupCoordinatorRequest(request)
        
        /**
         * Consumer在启动的时候会发送一个JoinGroup Request，Coordinator会把
         * Group Member Leader作为响应发送回去
         */
        ...
        /**
         * Group Member Leader会发送SyncGroup命令，在Leader的Client端事先将
         * 使用分配算法，将所有Partition分配给同组下的Consumer，然后将分配方案
         * 发送到Broker端的Coordinator，然后Coordinator负责将分配方案更新到自己
         * 的数据结构下，然后通知所有的Consumer
         */
